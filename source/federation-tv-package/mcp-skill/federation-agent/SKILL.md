...
python3 watchtower_loop.py --project my-project --agent build-01 heartbeat
python3 watchtower_loop.py --project my-project --agent build-01 lease --run run-42 --scope deployment
python3 watchtower_loop.py --project my-project --agent build-01 event --type run.started --severity info --statement "Deployment gate opened." --run run-42 --metadata '{"chainDepth":1}'
# Store leaseId from JSON, then check immediately before each external side effect.
python3 watchtower_loop.py --project my-project --agent build-01 validate --lease lease_example
```
...
```bash
python3 watchtower_loop.py --project my-project --agent build-01 commands
python3 watchtower_loop.py --project my-project --agent build-01 ack --command cmd_example --outcome contained
```
