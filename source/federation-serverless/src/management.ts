...
export async function handleManagementRequest(input: {
  ...
  if (orgGet && method === "GET") {
    ...
    const org = await env.DB.prepare("SELECT id, owner_id, name, contact_email, official_url, status, created_at, updated_at FROM federation_organizations WHERE id = ?").bind(orgId).first<OrgRow>();
...
interface OrgRow {
  id: string; owner_id: string; name: string; contact_email: string;
  official_url: string; status: string; created_at: number; updated_at: number;
}

export function presentOrg(o: OrgRow) {
  return {
    id: o.id, organizationId: o.id, ownerId: o.owner_id, name: o.name,
