...
export async function handleLifecycleRequest(input: {
  ...
  if (path === "/api/v1/agents" && method === "POST") {
    ...
    await env.DB.prepare("INSERT INTO federation_agents (id, project_id, agent_id, owner_id, organization_id, display_name, role, capabilities, avatar_seed, palette_key, character_type, public_projection, heartbeat_seconds, room_id, lifecycle_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'registered', ?, ?)")
      .bind(canonicalId, manifest.projectId, manifest.agentId, owner.id, manifest.organizationId || null, manifest.displayName, manifest.role, JSON.stringify(manifest.capabilities), manifest.identity.avatarSeed, manifest.identity.paletteKey, manifest.identity.characterType, manifest.publicProjection ? 1 : 0, manifest.heartbeatSeconds, legacy?.roomId || null, now, now).run();
