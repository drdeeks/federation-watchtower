...
(function(global) {
  ...
  class FederationTV {
    constructor(options = {}) {
      ...
      this.speechLines = [...DEFAULT_SPEECH_LINES, ...FEDERATION_REPERTOIRE];
    ...
    async init() {
      ...
      await this.fetchSpeechPool();
      await this.fetchEvents();
      this.start();
    }

    async fetchSpeechPool() {
      try {
        const res = await fetch(`${this.gatewayUrl}/api/federation/speech?limit=500`);
        if (!res.ok) return;
        const data = await res.json();
        const lines = (Array.isArray(data) ? data : (data.speechLines || data.lines || []));
        for (const item of lines) {
          const text = item.statement || item.text || item.message;
          if (text && !this.speechLines.includes(text)) {
            this.speechLines.push(text);
          }
        }
      } catch (e) {
        console.warn('[FederationTV] Speech pool fetch failed:', e);
      }
    }
    ...
    async fetchEvents() {
      try {
        ...
        for (const event of eventsToAnnounce) {
          ...
          if (event.agentId && event.eventType !== 'agentUpdated' && event.visibility?.publicBubble !== false && (event.statement || event.message)) {
    ...
    drainBubbleQueue() {
      ...
      const bubbleText = this.pickRepertoireLine(event);
      const delivered = this.showBubble(event.agentId, bubbleText);
    ...
    pickRepertoireLine(event) {
      const severity = (event.severity || event.priority || 'info').toLowerCase();
      const negative = ['error', 'critical', 'warning', 'high'].includes(severity);
      const pool = this.speechLines;
      if (pool.length === 0) return event.statement || event.message || '';
      // Pick random from tone-appropriate half of the pool.
      // Negative events draw from the first half (concerned lines),
      // positive from the second half (relaxed lines). Full pool fallback.
  ...
  // EXPORTS
  // ============================================================
  // Always expose the legacy engine under its own name so the tv-widget.js
  // loader can use it as the explicit fallback.
  global.FederationTVVanilla = FederationTV;
  // Only claim the public FederationTV name when a loader wrapper has not
  // already defined it. tv-widget.js prefers the React monitor and treats this
  // vanilla engine as the fallback; it must not be overwritten here.
  if (!global.FederationTV) global.FederationTV = FederationTV;

  // Auto-init only for pages that include tv-widget-vanilla.js DIRECTLY with
  // data attributes. When tv-widget.js injects this file programmatically it
  // constructs the engine itself, so this guard prevents a double-mount.
  function autoInit() {
    const scripts = document.querySelectorAll('script[src*="tv-widget-vanilla.js"][data-project]');
    scripts.forEach(script => {
      ...
      new FederationTV({ projectId, container: containerId, gatewayUrl });
    });
  }
