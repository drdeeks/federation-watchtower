# Chain Enforcement Integration — Skill Creator Pipeline
...
try:
    ...
    # ... for each gate ...
    subprocess.run(["python3", LOOP_ENFORCER, "verify", str(chain_workdir), chain_name, str(step_path)])
    subprocess.run(["python3", LOOP_ENFORCER, "complete", str(chain_workdir), chain_name, str(step_path)])
finally:
