...
- **No `config.yaml`** — the default profile uses the global `~/.hermes/config.yaml`, but new profiles get an empty directory. Workers spawned without config run blind.
...
wc -c ~/.hermes/profiles/<name>/config.yaml
...
for p in $(ls ~/.hermes/profiles/ | grep -v default); do
  [ ! -f ~/.hermes/profiles/$p/config.yaml ] && \
    cp ~/.hermes/config.yaml ~/.hermes/profiles/$p/config.yaml
  [ ! -d ~/.hermes/profiles/$p/skills ] && [ ! -L ~/.hermes/profiles/$p/skills ] && \
    ln -s ~/.hermes/skills ~/.hermes/profiles/$p/skills
done
```
...
```bash
sqlite3 ~/.hermes/kanban.db "SELECT id, status, assignee, title, worker_pid FROM tasks WHERE status IN ('running','ready');"
```

**Step 2: Kill worker processes**
```bash
for pid in $(sqlite3 ~/.hermes/kanban.db "SELECT worker_pid FROM tasks WHERE status='running' AND worker_pid IS NOT NULL;"); do
...
**Step 3: Cancel in DB**
```bash
sqlite3 ~/.hermes/kanban.db "UPDATE tasks SET status='cancelled', completed_at=strftime('%s','now') WHERE status IN ('running','ready');"
```
